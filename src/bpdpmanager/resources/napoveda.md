...
- **Pravý klik** na dokument otevře kontextové menu (funguje i u oponentur):
  *Otevřít* · *📂 Zobrazit ve Finderu* · *Odebrat*, a u **souborů** navíc:
  ...
  > pravý klik je můžeš **najednou *Otevřít*** (otevřou se všechny vybrané, ne
  > jen jeden), **hromadně exportovat** do zvolené složky, **odeslat jedním
  > e-mailem** (všechny jako přílohy) nebo **🗑 hromadně odebrat** (s jedním
  > dotazem, zda smazat i soubory ze složky).
...
posudku vedoucího** — **česky i anglicky** (české „Navržená známka: D",
anglické „suggest the following evaluation: B" / „suggest classification with
grade B"). Sekce Souhrnu
...
> *(Samotnou aktualizaci stavu řeší tichá kontrola po startu a kontextová
> akce „🔄 Aktualizace práce ze STAG…" nad prací — viz výše.)*
...
- PDF se ukládají **strukturovaně a přejmenovaná** do `komise/<rok>/rozpisy/`
  ...
  SZZ záznamy jejích studentů; studenty sdílené s jinou komisí zachová). Smazání
  se **pamatuje**, takže se komise **po restartu znovu nenaseeduje** (nevrátí se;
  *„🔄 Načíst komise znovu"* to vyresetuje). Zdrojová PDF na disku zůstanou.
