...
def test_statuses_only_updates_supervised_and_opposing(qapp, tmp_path, monkeypatch) -> None:
    """„Aktualizovat jen stavy" změní stav vedené práce i kód oponentury (bez souborů)."""
    ...
    # Vedená „V řešení" → STAG hlásí úspěšnou obhajobu (DUO) → Obhájeno.
    s = Student(first_name="Jan", last_name="Novák")
    svc.upsert_student(s)
    led = Thesis(type=ThesisType.BP, status=ThesisStatus.IN_PROGRESS,
                 academic_year="2024/2025", student_id=s.id, adipidno="111")
    svc.upsert_thesis(led)
    # Vedená „Nedokončeno" → STAG hlásí neúspěšnou obhajobu (DBUO) → Neobhájeno.
    s2 = Student(first_name="Petr", last_name="Svoboda")
    svc.upsert_student(s2)
    canc = Thesis(type=ThesisType.BP, status=ThesisStatus.CANCELLED,
                  academic_year="2020/2021", student_id=s2.id, adipidno="112")
    svc.upsert_thesis(canc)
    # Oponentura bez kódu → STAG hlásí ND.
    ...
    dlg = StagDownloadDialog(service=svc)
    dlg._checked_results = lambda: [
        stag_api.StagThesisResult(adipidno="111", surname="Novák", status_code="DUO"),
        stag_api.StagThesisResult(adipidno="112", surname="Svoboda", status_code="DBUO"),
        stag_api.StagThesisResult(adipidno="222", surname="Dvořák", status_code="ND"),
    ]
    dlg._update_statuses_only()

    assert svc.get_thesis(led.id).status == ThesisStatus.DEFENDED
    assert svc.get_thesis(canc.id).status == ThesisStatus.FAILED  # migrace na Neobhájeno
    assert svc.get_opposing_thesis(op.id).stag_state_code == "ND"
    assert dlg.statuses_only_done is True
